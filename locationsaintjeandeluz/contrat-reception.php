...
require('fpdf/fpdf.php');
		...
		if(!preg_match("#[^a-zA-Z0-9_.\-@]#", $_POST['mdp-inscription'])){//si il y a pas d'autres caract�res que cela
...
//inscription � la vol�e   ...  si tous les champs de l'inscription sont remplis et que l email est bon   rang 2
if((isset($_POST['sexe-inscription']))&&(!empty($_POST['nom-inscription']))&&(!empty($_POST['prenom-inscription']))&&(isset($pseudo_inscription))&&(isset($mdp_inscription))&&(!empty($_POST['mdp2-inscription']))&&($mdp_inscription==$_POST['mdp2-inscription'])){
	if (preg_match("#^[a-z0-9.-_]+@[a-z0-9.-_]{2,}\.[a-z]{2,4}$#" , $email_inscription)){
		...
		try{
			...
			die('Erreur : Connection � la base de donn�es impossible. R�essayez s\il vous plait' . $e->getMessage());}
			//requete1: v�rification email deja connu
			...
			if($donnee['email'] != $email_inscription){
				...
				if($donnee2['pseudo'] != $pseudo_inscription){
					...
					if(!$requete_insertion_membre){
						die( 'les donn�es n\'ont pas pu etre enregistrer. erreur de requete :'.mysql_error());
					}else{
						//d�but de l'envoie du mail de confirmation   spf non inclus
						// on g�re le saut � la ligne selon les serveurs
						...
						$message_html.="<p>Bonjour et bienvenue, <br />vous �tes � pr�sent membre de notre site.</p>";
						...
						$message_html.="<p>Utilisez l'espace membre pour modifier votre contrat dans la section \"mon contrat de location\" ou pour en cr�er un nouveau.<br />Si vous avez des probl�mes pour vous connecter, ne fa�tes pas de copier-coller.</p>";
						$message_html.="<p><a href='http://www.locationsaintjeandeluz.fr/contrat.html' title='cr�er mon contrat de location saisonni�re meubl�e gratuit'>Cr�ez votre contrat de location saisonni�re gratuit en ligne.</a></p>";
						...
						// -----fin header et d�but message
						//ajout 1�re boundary (fronti�re)
				...
				try{
					...
					die('Erreur : ' . $e->getMessage('Connection � la base de donn�e impossible. Veuillez fermer cette page et r�essayer s\il vous plait'));}
...
// fin de l'inscription � la vol�e
//test si e-mail pas pr�sent dans bdd
		try{
			$bdd = new PDO('mysql:host=localhost;dbname=','','');}//host=localhost;dbname=test','root','');}
		catch (Exception $e){
			die('Erreur : Connection � la base de donn�es impossible. R�essayez s\il vous plait' . $e->getMessage());}
...
//test si contrat et �tat dans la base.
...
//
//d�but de la cr�ation du pdf
//
class PDF extends FPDF{
function header(){
	$this->SetFont('Arial','U',18);
	$this->Cell(48);  //largeur totale du doc : 195
	$this->Cell(100,7,'Contrat de location saisonni�re',0,1,'C');
	$this->Ln(3);
	}
function footer(){
	$this->SetY(-12);
	$this->SetFont('Arial','',7);
	$this->Cell(60,3,'Mis � disposition gratuitement par locationsaintjeandeluz.fr',0,0,'L');
...
function info_proprio($txt1,$txt2,$txt3,$txt4){
	$this->intit_proprio('Le(s) propri�taire(s) d�nomm� "le propri�taire" :');
	$this->nom_proprio($txt1);
	$this->intit_adresse('Adresse :');
	$this->adresse($txt2);
	$this->intit_telephone('T�l�phone :');
	$this->telephone($txt3);
...
function info_locataires($txt1,$txt2,$txt3,$txt4){
	$this->intit_locataires('Le(s) locataire(s) d�nomm� "le locataire" :');
	$this->nom_locataires($txt1);
	$this->intit_adresse_locataires('Adresse :');
	$this->adresse_locataires($txt2);
	$this->intit_telephone_locataires('T�l�phone :');
...
function adresse_location($txt){
	$this->intit_adresse_location('Adresse pr�cise :');
...
function intit_nbre_pieces(){
	...
	$this->SetFont('Arial','',9);
	$this->Cell(31,3,'- Nombre de pi�ces :',0,0);
	}
...
function intit_fumeur(){
	...
	$this->SetFont('Arial','',9);
	$this->Cell(32,3,'- Fumeurs accept�s :',0,0);
	}
...
function intit_animaux(){
	...
	$this->SetFont('Arial','',9);
	$this->Cell(32,3,'- Animaux accept�s :',0,0);
	}
...
function salon($txt){
	$this->intit_salon('Salon/salle � manger :');
...
function autres($txt){
	$this->intit_autres('Autres (garage, jardin, situation g�ographique...) :');
...
function contenu_date($txt1,$txt2,$txt3,$txt4){
	...
	$this->Cell(190,4,'Location entendue du '.$txt1.' � '.$txt2.' heures jusqu\'au '.$txt3.' � '.$txt4.' heures.',0,1,'C');
	}
...
function contenu_prix($txt1,$txt2,$txt3,$txt4,$txt5,$txt6){
	$this->SetFont('Arial','',11);
	$this->MultiCell(190,4,'Le prix de cette location est de '.$txt1.'� par semaine, non inclus les charges mentionn�es plus bas.
		Les arrhes de cette location sont de '.$txt2.'% soit '.$txt3.'� et seront vers�es lors de la signature du pr�sent contrat.
		Le solde du prix, soit '.$txt4.'� sera pay� le jour de la remise des cl�s, en m�me temps que le d�p�t de
		garantie de '.$txt5.'% soit '.$txt6.'�. Ce d�p�t de garantie peut etre encaiss�. Il sera rendu en int�gralit� jusqu\'� quinze jours apr�s le d�part du locataire, except� en cas de d�g�ts.',0,'C');
	}
function prix($txt1,$txt2,$txt3,$txt4,$txt5,$txt6){
	$this->SetY(48);
	$this->intit_prix('Prix, arrhes et d�p�t de garantie :');
	$this->Ln();
...
function condition_paiement($txt){
	...
	$this->Ln();
	$this->SetFont('Arial','',11);
	$this->MultiCell(190,4,'Le locataire paiera par tout moyen convenu avec le propri�taire et mentionn� ici: '.$txt.'.',0,'C');
	}
...
function condition_annulation(){
	...
	$this->Ln();
	$this->SetFont('Arial','',11);
	$this->MultiCell(190,4,'Les deux parties ont sept jours pour se r�tracter � partir de la date de signature, sans qu\'aucune
	p�nalit� ne leur soit inflig�e.',0,'C');
	$this->Cell(190,4,'Pass� ce d�lai :',0,1,'C');
	$this->MultiCell(190,4,'- Le locataire peut se r�tracter � tout moment mais perdra l\'int�gralit� de ses arrhes.',0,'C');
	$this->MultiCell(190,4,'- Le propri�taire peut se r�tracter � tout moment mais devra remettre le double des arrhes d�j� vers�es par le locataire.',0,'C');
	}
// charges ++
function intit_charges(){
	$this->SetFont('Arial','B',14);
	$this->Cell(190,4,'Charges suppl�mentaires :',0,1,'C');
	}
function charges($txt1,$txt2,$txt3){
	...
	$this->Ln();
	$this->SetFont('Arial','',11);
	$this->Cell(190,4,'D\'autres charges peuvent �tre demand�es :',0,1,'C');
	$this->Cell(190,4,'Taxe de s�jour : '.$txt1.'',0,1,'C');
	$this->Cell(190,4,'Eau/�l�ctricit�/gaz : '.$txt2.'',0,1,'C');
...
function obligations(){
	...
	$this->Cell(190,4,'- Assumer sa responsabilit� civile.',0,1,'C');
	$this->Cell(190,4,'- Utiliser le logement "raisonnablement" et r�pondre des d�gradations dans les plus brefs d�lais.',0,1,'C');
	$this->Cell(190,4,'- Respecter la capacit� maximale d\'h�bergement.',0,1,'C');
	$this->Cell(190,4,'- Faire le m�nage si aucun forfait n\'est pr�vu, et rendre le bien en location dans le m�me �tat de propret�',0,1,'C');
	$this->Cell(190,4,'que lors de son entr�e dans les lieux.',0,1,'C');
	}
function annexes(){
	$this->SetY(207);
	$this->SetFont('Arial','B',14);
	...
	$this->Ln();
	$this->SetFont('Arial','',11);
	$this->MultiCell(190,4,'Vous trouverez en annexe � ce contrat le dossier de diagnostic technique comprenant, si n�cessaire, un 
	constat de risque d\'exposition au plomb, un �tat des risques naturels et technologiques, 
	ainsi qu\'un diagnostic de performances �nerg�tiques.',0,'C');
	}
//signature
function signature($txt1){
	$this->SetY(231);
	$this->Cell(30,5,'',0,0);
	$this->Cell(40,5,'Fait en deux exemplaires � : '.$txt1.'   le' ,0,1,'L');
	$this->Ln();
	$this->Cell(190,4,'Signatures des deux parties pr�c�d�es de la mention "lu et approuv�"',0,1,'C');
	$this->Cell(95,5,'Le(s) propri�taire(s)',0,0,'C');
...
//
//Fin de la condition generale et EMAIL du d�but
